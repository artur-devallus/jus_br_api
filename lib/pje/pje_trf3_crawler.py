...
class PjeTrf3Crawler:
    ...
    def process(self, main_soup) -> ProcessData:
        return ProcessData(
            process_number=self.property_value(
                main_soup.find('span', attrs=dict(id='j_id145:processoTrfViewView:j_id153'))
            ),
            distribution_date=to_date(self.property_value(
                main_soup.find('span', attrs=dict(id='j_id145:processoTrfViewView:j_id165'))
            )),
            jurisdiction=self.property_value(
                main_soup.find('span', attrs=dict(id='j_id145:processoTrfViewView:j_id200'))
            ),
            judicial_class=self.property_value(
                main_soup.find('span', attrs=dict(id='j_id145:processoTrfViewView:j_id176'))
            ),
            judge_entity=self.property_value(
                main_soup.find('span', attrs=dict(id='j_id145:processoTrfViewView:j_id224'))
            ),
            collegiate_judge_entity=self.property_value(
                main_soup.find('span', attrs=dict(id='j_id145:processoTrfViewView:j_id211'))
            ),
            referenced_process_number=self.property_value(
                main_soup.find('span', attrs=dict(id='j_id145:processoTrfViewView:j_id237'))
            ),
            subject=self.property_value(
                main_soup.find('span', attrs=dict(id='j_id145:processoTrfViewView:j_id187'))
            ),
        )
    ...
    def _polo_ativo_soup(self, page, referer: str) -> BeautifulSoup:
        data = {
            "AJAXREQUEST": "_viewRoot",
            'j_id145:processoPartesPoloAtivoResumidoList:j_id336': 'j_id145:processoPartesPoloAtivoResumidoList:j_id336',
            'javax.faces.ViewState': f'j_id{self._current_id}',
            'ajaxSingle': 'j_id145:processoPartesPoloAtivoResumidoList:j_id336:j_id337',
            'j_id145:processoPartesPoloAtivoResumidoList:j_id336:j_id337': str(page),
            "AJAX:EVENTS_COUNT": "1"
        }
        url = f'{self.BASE_URL}/pje/ConsultaPublica/DetalheProcessoConsultaPublica/listView.seam'
    ...
    def _passive_parties_soup(self, page, referer: str) -> BeautifulSoup:
        data = {
            "AJAXREQUEST": "_viewRoot",
            'j_id145:processoPartesPoloPassivoResumidoList:j_id400': 'j_id145:processoPartesPoloPassivoResumidoList:j_id400',
            'javax.faces.ViewState': f'j_id{self._current_id}',
            'ajaxSingle': 'j_id145:processoPartesPoloPassivoResumidoList:j_id400:j_id401',
            'j_id145:processoPartesPoloPassivoResumidoList:j_id400:j_id401': str(page),
            "AJAX:EVENTS_COUNT": "1"
        }
        url = f'{self.BASE_URL}/pje/ConsultaPublica/DetalheProcessoConsultaPublica/listView.seam'
    ...
    def _movements_soup(self, page, referer: str) -> BeautifulSoup:
        data = {
            "AJAXREQUEST": "j_id145:j_id473",
            "j_id145:j_id556:j_id557": str(page),
            'j_id145:j_id556': 'j_id145:j_id556',
            'autoScroll': '',
            'javax.faces.ViewStfate': f'j_id{self._current_id}',
            'j_id145:j_id556:j_id558': 'j_id145:j_id556:j_id558',
            "AJAX:EVENTS_COUNT": "1"
        }
    ...
    def polo_ativo(self, main_soup: BeautifulSoup, referer: str) -> List[Party]:
        extract_rows_fn = lambda soup: [
            ...
            for x in soup.find(
                'tbody', attrs=dict(id='j_id145:processoPartesPoloAtivoResumidoList:tb')
            ).find_all('tr')
        ]
    ...
    def polo_passivo(self, main_soup: BeautifulSoup, referer: str) -> List[Party]:
        extract_rows_fn = lambda soup: [
            self._party_from_text(x.find('td').text)
            for x in soup.find(
                'tbody', attrs=dict(id='j_id145:processoPartesPoloPassivoResumidoList:tb')
            ).find_all('tr')
        ]
    ...
    def movements(self, main_soup: BeautifulSoup, referer: str) -> List[Movement]:
        extract_movements_fn = lambda soup: [
            ...
            for x in soup.find(
                'tbody', attrs=dict(id='j_id145:processoEvento:tb')
            ).find_all('tr')
        ]
