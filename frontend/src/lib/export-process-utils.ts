  ...
  for (const q of allQueries) {
    ...
    for (const proc of query.processes) {
      ...
      wsData.push([
        ...
        `${proc.activePartyAuthorCpf()}`,
        `${proc.activePartyAuthorName()}`,
        `${proc.activePartyLawyerCpf()}`,
        `${proc.activePartyLawyerName()}`,
        `${proc.passivePartyDefendantCpfOrCnpj()}`,
        `${proc.passivePartyDefendantName()}`,
        proc.subject() || "-",
